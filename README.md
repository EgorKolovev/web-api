...
в проекте `WebApi.MinimalApi`. Эти методы должны позволять создавать, удалять и обновлять пользователей,
получать информацию о имеющихся пользователях.

Для начала надо научиться запускать `WebApi.MinimalApi` под отладкой удобным тебе способом в удобной тебе IDE.
